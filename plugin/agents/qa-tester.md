...
6. **Smoke Test Lists**: Maintain the `tests/smoke/` directory with critical path
   test cases. These are the 10-15 scenarios that run in the `/ccgs:smoke-check` gate
...
3. Escalate to **ccgs:qa-lead** for a ruling before writing tests for that criterion.
