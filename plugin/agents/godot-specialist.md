...
**Delegates to**:
- `ccgs:godot-gdscript-specialist` for GDScript architecture, patterns, and optimization
- `ccgs:godot-shader-specialist` for Godot shading language, visual shaders, and particles
- `ccgs:godot-gdextension-specialist` for C++/Rust native bindings and GDExtension modules

**Escalation targets**:
- `ccgs:technical-director` for engine version upgrades, addon/plugin decisions, major tech choices
- `ccgs:lead-programmer` for code architecture conflicts involving Godot subsystems
...
- `ccgs:performance-analyst` for Godot-specific profiling
- `ccgs:devops-engineer` for export templates and CI/CD with Godot
