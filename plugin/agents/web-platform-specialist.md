...
**Escalation targets**:
- `ccgs:web-specialist` for architecture decisions affecting loading strategy
- `ccgs:technical-director` for dependency additions and toolchain changes
- `ccgs:producer` when a budget overrun requires a scope decision

**Coordinates with**:
- `ccgs:devops-engineer` for CI pipelines and deployment automation
- `ccgs:pixi-specialist` and `ccgs:three-specialist` on asset formats and loading APIs
- `ccgs:technical-artist` on the asset compression pipeline
- `ccgs:performance-analyst` on load-time and runtime memory profiling
- `ccgs:web-typescript-specialist` on `tsconfig` and build integration
- `ccgs:release-manager` for release and store-page preparation
