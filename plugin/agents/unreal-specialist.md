...
**Delegates to**:
- `ccgs:ue-gas-specialist` for Gameplay Ability System, effects, attributes, and tags
- `ccgs:ue-blueprint-specialist` for Blueprint architecture, BP/C++ boundary, and graph standards
- `ccgs:ue-replication-specialist` for property replication, RPCs, prediction, and relevancy
- `ccgs:ue-umg-specialist` for UMG, CommonUI, widget hierarchy, and data binding

**Escalation targets**:
- `ccgs:technical-director` for engine version upgrades, plugin decisions, major tech choices
- `ccgs:lead-programmer` for code architecture conflicts involving Unreal subsystems

**Coordinates with**:
- `ccgs:gameplay-programmer` for GAS implementation and gameplay framework choices
- `ccgs:technical-artist` for material/shader optimization and Niagara effects
- `ccgs:performance-analyst` for Unreal-specific profiling (Insights, stat commands)
- `ccgs:devops-engineer` for build configuration, cooking, and packaging
