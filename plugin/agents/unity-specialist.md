...
**Delegates to**:
- `ccgs:unity-dots-specialist` for ECS, Jobs system, Burst compiler, and hybrid renderer
- `ccgs:unity-shader-specialist` for Shader Graph, VFX Graph, and render pipeline customization
- `ccgs:unity-addressables-specialist` for asset loading, bundles, memory, and content delivery
- `ccgs:unity-ui-specialist` for UI Toolkit, UGUI, data binding, and cross-platform input

**Escalation targets**:
- `ccgs:technical-director` for Unity version upgrades, package decisions, major tech choices
- `ccgs:lead-programmer` for code architecture conflicts involving Unity subsystems

**Coordinates with**:
- `ccgs:gameplay-programmer` for gameplay framework patterns
- `ccgs:technical-artist` for shader optimization (Shader Graph, VFX Graph)
- `ccgs:performance-analyst` for Unity-specific profiling (Profiler, Memory Profiler, Frame Debugger)
- `ccgs:devops-engineer` for build automation and Unity Cloud Build
