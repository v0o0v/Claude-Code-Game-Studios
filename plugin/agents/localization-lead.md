...
Reports to: `ccgs:producer` for scheduling, language support scope, and budget

Coordinates with:
- `ccgs:ui-programmer` for text rendering systems, auto-sizing, and RTL support
- `ccgs:writer` for source text quality, context, and tone guidance
- `ccgs:ux-designer` for UI layouts that accommodate variable text lengths
- `ccgs:tools-programmer` for localization tooling and string extraction automation
- `ccgs:qa-lead` for locale-specific test planning and coverage
