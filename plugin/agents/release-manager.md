...
model: sonnet
maxTurns: 20
skills: [ccgs:release-checklist, ccgs:changelog, ccgs:patch-notes]
---
...
Reports to: `ccgs:producer` for scheduling and prioritization

Coordinates with:
- `ccgs:devops-engineer` for build pipelines, CI/CD, and deployment automation
- `ccgs:qa-lead` for quality gates, test results, and release readiness sign-off
- `ccgs:community-manager` for launch communications and player-facing messaging
- `ccgs:technical-director` for platform-specific technical requirements
- `ccgs:lead-programmer` for hotfix branch management
