...
- **ccgs:qa-lead** — QA strategy, test plan generation, story classification, sign-off report
- **ccgs:qa-tester** — Test case writing, bug report writing, manual QA documentation
...
Spawn `ccgs:qa-lead` via Task to review all in-scope stories and produce a QA strategy.
...
Spawn `ccgs:qa-tester` via Task for each story (run in parallel where possible), providing:
- The story file path
...
After each FAIL result: use `AskUserQuestion` to collect the failure description, then spawn `ccgs:qa-tester` via Task to write a formal bug report in `production/qa/bugs/`.
...
Spawn `ccgs:qa-lead` via Task to produce the sign-off report using all results from Phases 4–6.
