...
> Usage: `/ccgs:team-audio [feature or area]` — specify the feature or area to design audio for (e.g., `combat`, `main menu`, `forest biome`, `boss encounter`). Do not use `AskUserQuestion` here; output the guidance directly.
...
Spawn the `ccgs:audio-director` agent to:
...
Spawn the `ccgs:sound-designer` agent to:
...
Spawn the `ccgs:technical-artist` agent to:
...
Spawn the `ccgs:gameplay-programmer` agent to:
