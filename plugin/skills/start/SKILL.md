...
3. If `TOOL_SPEC.md` already exists:
   ...
   - Suggest `/ccgs:code-review tools/` or just continuing development
4. If no spec exists yet, recommend `/ccgs:setup-tool` as the first step
5. Show the recommended path:
   - `/ccgs:setup-tool` — define the tool's purpose, I/O, and tech stack
   - `/ccgs:reverse-document` — if the tool already exists, generate docs from the code
   - `/ccgs:code-review tools/` — review quality of existing scripts
   - `/ccgs:architecture-decision` — record significant design choices
