...
**After pillars and anti-pillars are agreed, spawn BOTH `ccgs:creative-director` AND `ccgs:art-director` via Task in parallel before moving to Phase 5. Issue both Task calls simultaneously — do not wait for one before starting the other.**

- **`ccgs:creative-director`** — gate **CD-PILLARS** (`${CLAUDE_PLUGIN_ROOT}/docs/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`ccgs:art-director`** — gate **AD-CONCEPT-VISUAL** (`${CLAUDE_PLUGIN_ROOT}/docs/director-gates.md`)
...
**After identifying biggest technical risks, spawn `ccgs:technical-director` via Task using gate TD-FEASIBILITY (`${CLAUDE_PLUGIN_ROOT}/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `ccgs:producer` via Task using gate PR-SCOPE (`${CLAUDE_PLUGIN_ROOT}/docs/director-gates.md`).**
...
4. **Generate the game concept document** using the template at
   `${CLAUDE_PLUGIN_ROOT}/templates/game-concept.md`. Fill in ALL sections from the
...
If yes, generate the document using the template at `${CLAUDE_PLUGIN_ROOT}/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
...
**Path A — Design-First** (recommended if the concept is well-defined):
   ...
   4. "Discuss vision with the `ccgs:creative-director` agent for pillar refinement"
