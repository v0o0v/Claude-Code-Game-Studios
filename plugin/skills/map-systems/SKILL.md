...
**After dependency mapping is approved, spawn `ccgs:technical-director` via Task using gate TD-SYSTEM-BOUNDARY (`${CLAUDE_PLUGIN_ROOT}/docs/director-gates.md`) before proceeding to priority assignment.**
...
**After priorities are approved, spawn `ccgs:producer` via Task using gate PR-SCOPE (`${CLAUDE_PLUGIN_ROOT}/docs/director-gates.md`) before writing the index.**
...
Using the template at `${CLAUDE_PLUGIN_ROOT}/templates/systems-index.md`, populate the
...
**After the systems index is written, spawn `ccgs:creative-director` via Task using gate CD-SYSTEMS (`${CLAUDE_PLUGIN_ROOT}/docs/director-gates.md`).**
...
After `/ccgs:design-system` completes, use `AskUserQuestion`:
