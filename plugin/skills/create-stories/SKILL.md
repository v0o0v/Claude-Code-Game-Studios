...
in this skill — apply the check pattern from `${CLAUDE_PLUGIN_ROOT}/docs/director-gates.md`
...
- **ADR Version**: for the primary governing ADR, embed the ADR file's last commit date next to the ADR reference. Use `git log -1 --format=%cs -- docs/architecture/[adr-file].md`. If the ADR has no git history yet, write `ADR Version: uncommitted` and note that `/ccgs:dev-story` will fall back to targeted ADR section reads.
...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `ccgs:qa-lead` via Task using gate **QL-STORY-READY** (`${CLAUDE_PLUGIN_ROOT}/docs/director-gates.md`).
