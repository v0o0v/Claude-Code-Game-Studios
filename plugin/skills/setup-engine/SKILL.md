...
```markdown
## Engine Specialists
- **Primary**: ccgs:unity-specialist
- **Language/Code Specialist**: ccgs:unity-specialist (C# review — primary covers it)
- **Shader Specialist**: ccgs:unity-shader-specialist (Shader Graph, HLSL, URP/HDRP materials)
- **UI Specialist**: ccgs:unity-ui-specialist (UI Toolkit UXML/USS, UGUI Canvas, runtime UI)
- **Additional Specialists**: ccgs:unity-dots-specialist (ECS, Jobs system, Burst compiler), ccgs:unity-addressables-specialist (asset loading, memory management, content catalogs)
...
|-----------------------|---------------------|
| Game code (.cs files) | ccgs:unity-specialist |
| Shader / material files (.shader, .shadergraph, .mat) | ccgs:unity-shader-specialist |
| UI / screen files (.uxml, .uss, Canvas prefabs) | ccgs:unity-ui-specialist |
| Scene / prefab / level files (.unity, .prefab) | ccgs:unity-specialist |
| Native extension / plugin files (.dll, native plugins) | ccgs:unity-specialist |
| General architecture review | ccgs:unity-specialist |
```

**For Unreal:**
```markdown
## Engine Specialists
- **Primary**: ccgs:unreal-specialist
- **Language/Code Specialist**: ccgs:ue-blueprint-specialist (Blueprint graphs) or ccgs:unreal-specialist (C++)
- **Shader Specialist**: ccgs:unreal-specialist (no dedicated shader specialist — primary covers materials)
- **UI Specialist**: ccgs:ue-umg-specialist (UMG widgets, CommonUI, input routing, widget styling)
- **Additional Specialists**: ccgs:ue-gas-specialist (Gameplay Ability System, attributes, gameplay effects), ccgs:ue-replication-specialist (property replication, RPCs, client prediction, netcode)
...
| File Extension / Type | Specialist to Spawn |
|-----------------------|---------------------|
| Game code (.cpp, .h files) | ccgs:unreal-specialist |
| Shader / material files (.usf, .ush, Material assets) | ccgs:unreal-specialist |
| UI / screen files (.umg, UMG Widget Blueprints) | ccgs:ue-umg-specialist |
| Scene / prefab / level files (.umap, .uasset) | ccgs:unreal-specialist |
| Native extension / plugin files (Plugin .uplugin, modules) | ccgs:unreal-specialist |
| Blueprint graphs (.uasset BP classes) | ccgs:ue-blueprint-specialist |
| General architecture review | ccgs:unreal-specialist |
```
...
### Step 7a: Copy the Bundled Reference Pack (baseline)

The plugin ships a curated, version-pinned reference snapshot for each of the
four supported engines at `${CLAUDE_PLUGIN_ROOT}/engine-reference/<engine>/`
(godot, unity, unreal, web). This is the starting baseline — always copy it
into the project before deciding whether anything further is needed.

1. Copy `${CLAUDE_PLUGIN_ROOT}/engine-reference/<engine>/` (the whole
   directory — `VERSION.md`, `breaking-changes.md`, `deprecated-apis.md`,
   `current-best-practices.md`, `modules/*.md`, and for Unity/Unreal also
   `PLUGINS.md` and `plugins/*.md`) into `docs/engine-reference/<engine>/`
   in the project. Also copy `${CLAUDE_PLUGIN_ROOT}/engine-reference/README.md`
   to `docs/engine-reference/README.md` if that file doesn't already exist.
   - If `docs/engine-reference/<engine>/` already exists with content, do
     NOT silently overwrite it — ask first (per the existing guardrail
     "NEVER overwrite existing reference docs without asking").
   - **Only copy the pack matching the chosen engine.** Do not copy the
     other three engines' packs into the project.
2. Read the copied `VERSION.md` to see what engine version and "Project
   Pinned" / "Last verified" date the bundled snapshot was written for.
3. Compare against the user's chosen version and today's date:
   - **Bundled pack's pinned version matches (or is very close to) the
     chosen version, and the pinned date is recent** → the baseline is
     good as-is. Skip to Section 8. This is the common case and needs no
     WebSearch.
   - **Bundled pack's version differs from the chosen version, or the
     pinned date is stale** → treat this the same as `/ccgs:setup-engine
     refresh` (Section 10): use WebSearch/WebFetch to update the copied
     files in place (don't recreate from scratch — refine what's there).
     Continue to Step 7b below.

If the chosen engine somehow has no bundled pack (should not happen — all
four supported engines have one), fall back to generating the full set from
scratch using Step 7b directly.

### Step 7b: Refresh Against Current Sources (only if the baseline is stale)

Re-run the version/knowledge-gap assessment from Section 6 against the
now-copied baseline, and use WebSearch to bring it up to date:

Search the web to update the copied reference set:
...
Ask: "The bundled reference pack for [engine] is pinned to [bundled version]
as of [bundled date], but you're on [chosen version] — may I refresh
`docs/engine-reference/<engine>/` with current data before we continue?"

Wait for confirmation before writing any files.

3. **Ensure the reference directory has these files**, creating any that are
   still missing after the Step 7a copy (normally all of them already exist):
   ```
...
4. **Update each file** using real data from the web searches, editing the
   bundled content in place rather than discarding it — merge new findings
   with what the pack already had, following the format already established
   in the copied docs. Every file must have a "Last verified: [date]" header.

5. **For module files**: Only create new modules for subsystems where
   significant changes occurred since the bundled pack was written. Don't
   create empty or minimal module files.
...
- When the user chose **GDScript**: copy the GDScript CLAUDE.md template from Appendix A1 exactly. NEVER add "C++ via GDExtension" to the Language field. GDScript projects may use GDExtension, but it is not a primary project language. The `ccgs:godot-gdextension-specialist` in the routing table is available for when native extensions are needed — it does not make C++ a project language.
...
- **Language/Code Specialist**: ccgs:godot-gdscript-specialist (all .gd files)
- **Shader Specialist**: ccgs:godot-shader-specialist (.gdshader files, VisualShader resources)
- **UI Specialist**: ccgs:godot-specialist (no dedicated UI specialist — primary covers all UI)
- **Additional Specialists**: ccgs:godot-gdextension-specialist (GDExtension / native C++ bindings only)
...
| File Extension / Type | Specialist to Spawn |
|-----------------------|---------------------|
| Game code (.gd files) | ccgs:godot-gdscript-specialist |
| Shader / material files (.gdshader, VisualShader) | ccgs:godot-shader-specialist |
| UI / screen files (Control nodes, CanvasLayer) | ccgs:godot-specialist |
| Scene / prefab / level files (.tscn, .tres) | ccgs:godot-specialist |
| Native extension / plugin files (.gdextension, C++) | ccgs:godot-gdextension-specialist |
| General architecture review | ccgs:godot-specialist |
```
...
## Engine Specialists
- **Primary**: ccgs:godot-specialist
- **Language/Code Specialist**: ccgs:godot-csharp-specialist (all .cs files)
- **Shader Specialist**: ccgs:godot-shader-specialist (.gdshader files, VisualShader resources)
- **UI Specialist**: ccgs:godot-specialist (no dedicated UI specialist — primary covers all UI)
- **Additional Specialists**: ccgs:godot-gdextension-specialist (GDExtension / native C++ bindings only)
...
| File Extension / Type | Specialist to Spawn |
|-----------------------|---------------------|
| Game code (.cs files) | ccgs:godot-csharp-specialist |
| Shader / material files (.gdshader, VisualShader) | ccgs:godot-shader-specialist |
| UI / screen files (Control nodes, CanvasLayer) | ccgs:godot-specialist |
| Scene / prefab / level files (.tscn, .tres) | ccgs:godot-specialist |
| Project config (.csproj, NuGet) | ccgs:godot-csharp-specialist |
| Native extension / plugin files (.gdextension, C++) | ccgs:godot-gdextension-specialist |
| General architecture review | ccgs:godot-specialist |
```
...
## Engine Specialists
- **Primary**: ccgs:godot-specialist
- **GDScript Specialist**: ccgs:godot-gdscript-specialist (.gd files — gameplay/UI scripts)
- **C# Specialist**: ccgs:godot-csharp-specialist (.cs files — performance-critical systems)
- **Shader Specialist**: ccgs:godot-shader-specialist (.gdshader files, VisualShader resources)
- **UI Specialist**: ccgs:godot-specialist (no dedicated UI specialist — primary covers all UI)
- **Additional Specialists**: ccgs:godot-gdextension-specialist (GDExtension / native C++ bindings only)
...
| Game code (.gd files) | ccgs:godot-gdscript-specialist |
| Game code (.cs files) | ccgs:godot-csharp-specialist |
| Cross-language boundary decisions | ccgs:godot-specialist |
| Shader / material files (.gdshader, VisualShader) | ccgs:godot-shader-specialist |
| UI / screen files (Control nodes, CanvasLayer) | ccgs:godot-specialist |
| Scene / prefab / level files (.tscn, .tres) | ccgs:godot-specialist |
| Project config (.csproj, NuGet) | ccgs:godot-csharp-specialist |
...
```markdown
## Engine Specialists
- **Primary**: ccgs:web-specialist
- **Language/Code Specialist**: ccgs:web-typescript-specialist (all .ts/.tsx files)
- **Renderer Specialist**: ccgs:pixi-specialist (all PixiJS rendering code)
- **Shader Specialist**: ccgs:web-shader-specialist (GLSL/WGSL, Pixi filters, post-processing)
- **UI Specialist**: ccgs:web-ui-specialist (DOM overlay UI, input, accessibility, responsive)
- **Additional Specialists**: ccgs:web-platform-specialist (Vite, bundling, asset streaming, budgets, deployment)
...
- **Primary**: ccgs:web-specialist
- **Language/Code Specialist**: ccgs:web-typescript-specialist (all .ts/.tsx files)
- **Renderer Specialist**: ccgs:three-specialist (all Three.js rendering code)
- **Shader Specialist**: ccgs:web-shader-specialist (TSL/NodeMaterial, GLSL/WGSL, RenderPipeline)
- **UI Specialist**: ccgs:web-ui-specialist (DOM overlay UI, input, accessibility, responsive)
- **Additional Specialists**: ccgs:web-platform-specialist (Vite, bundling, asset streaming, budgets, deployment)
...
- **Primary**: ccgs:web-specialist
- **Language/Code Specialist**: ccgs:web-typescript-specialist (all .ts/.tsx files)
- **3D Renderer Specialist**: ccgs:three-specialist (Three.js scene, materials, GLTF)
- **2D Renderer Specialist**: ccgs:pixi-specialist (PixiJS HUD and 2D overlay layers)
- **Shader Specialist**: ccgs:web-shader-specialist (TSL, GLSL/WGSL, Pixi filters, post-processing)
- **UI Specialist**: ccgs:web-ui-specialist (DOM overlay UI, input, accessibility, responsive)
- **Additional Specialists**: ccgs:web-platform-specialist (Vite, bundling, asset streaming, budgets, deployment)
...
| File Extension / Type | Specialist to Spawn |
|-----------------------|---------------------|
| `.ts` importing `three` | ccgs:three-specialist |
| `.ts` importing `pixi.js` | ccgs:pixi-specialist |
| `src/ui/**` (.ts, .tsx, .html, .css) | ccgs:web-ui-specialist |
| Build config (vite.config.ts, package.json, tsconfig.json) | ccgs:web-platform-specialist |
| Web Workers (.worker.ts) | ccgs:web-platform-specialist |
| Shader files (.glsl, .wgsl, .frag, .vert) | ccgs:web-shader-specialist |
| All other game code (.ts, .tsx) | ccgs:web-typescript-specialist |
| Scene / level data (.json, level definitions) | ccgs:web-specialist |
| General architecture review | ccgs:web-specialist |
