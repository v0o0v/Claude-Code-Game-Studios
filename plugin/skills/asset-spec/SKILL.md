...
**Mode behavior:**
- `full` (default): spawn both `ccgs:art-director` and `ccgs:technical-artist` in parallel
- `lean`: spawn `ccgs:art-director` only — faster, skips technical constraint pass
...
**`ccgs:technical-artist`** via Task:
...
2. In `lean` mode or if `ccgs:technical-artist` blocks: proceed with art-director output only — note that technical constraints were not validated
3. In `solo` mode or if `ccgs:art-director` blocks: derive descriptions from art bible rules — flag as "Art director not consulted — verify against art bible before production"
