...
- **ADR Decision Summary** — the distilled decision embedded by `/ccgs:create-stories`
- **Implementation Notes** — the ADR guidance section embedded by `/ccgs:create-stories`
...
### The governing ADR
Do **not** read the full ADR by default. `/ccgs:create-stories` already embeds the
...
1. Check the ADR header/status first. If `Status:` is not `Accepted`, output
   **BLOCKED** and recommend `/ccgs:architecture-decision`.
...
|---|---|
| Foundation layer — any type | `ccgs:engine-programmer` |
| Any layer — Type: UI | `ccgs:ui-programmer` |
| Any layer — Type: Visual/Feel | `ccgs:gameplay-programmer` (implements) |
| Core or Feature — gameplay mechanics | `ccgs:gameplay-programmer` |
| Core or Feature — AI behaviour, pathfinding | `ccgs:ai-programmer` |
| Core or Feature — networking, replication | `ccgs:network-programmer` |
...
|--------|----------------------------|
| Godot 4 | `ccgs:godot-specialist`, `ccgs:godot-gdscript-specialist`, `ccgs:godot-shader-specialist` |
| Unity | `ccgs:unity-specialist`, `ccgs:unity-ui-specialist`, `ccgs:unity-shader-specialist` |
| Unreal Engine | `ccgs:unreal-specialist`, `ccgs:ue-gas-specialist`, `ccgs:ue-blueprint-specialist`, `ccgs:ue-umg-specialist`, `ccgs:ue-replication-specialist` |
| Web (PixiJS / Three.js) | `ccgs:web-specialist`, `ccgs:web-typescript-specialist`, `ccgs:pixi-specialist`, `ccgs:three-specialist`, `ccgs:web-shader-specialist`, `ccgs:web-ui-specialist`, `ccgs:web-platform-specialist` |
...
> imports `three` → `ccgs:three-specialist`; imports `pixi.js` → `ccgs:pixi-specialist`;
> `src/ui/**` → `ccgs:web-ui-specialist`; build config or `.worker.ts` →
> `ccgs:web-platform-specialist`; shaders → `ccgs:web-shader-specialist`; everything else
> → `ccgs:web-typescript-specialist`. See Appendix B of `/ccgs:setup-engine`.
...
Spawn `ccgs:gameplay-programmer` to implement the code/animation calls. Note that
