...
Spawn `ccgs:localization-lead` via Task. Ask them to audit the following for cultural sensitivity across the target locales (read from `assets/data/strings/` and `assets/`):
...
Spawn `ccgs:localization-lead` via Task with:
