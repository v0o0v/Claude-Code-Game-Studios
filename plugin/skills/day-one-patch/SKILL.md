...
Spawn `ccgs:release-manager` via Task. Ask them to produce a rollback plan covering:
...
1. Spawn `ccgs:lead-programmer` via Task with:
...
3. Spawn `ccgs:qa-tester` via Task to verify: does the bug reproduce after the fix?
...
Spawn `ccgs:qa-lead` via Task with:
