...
- **Named**: `/ccgs:setup-tool [tool name]` — tool name provided, guided questions follow
- **No args**: `/ccgs:setup-tool` — fully guided mode
...
- Are there existing scripts in `tools/`? → Offer to run `/ccgs:reverse-document` afterward
...
1. If the tool already exists:
   → /ccgs:reverse-document to generate architecture docs from existing code
...
2. If starting from scratch:
   ...
   → Run /ccgs:code-review once the first working version exists

3. To document design decisions as you build:
   → /ccgs:architecture-decision [decision name]
...
- If the user has existing code, strongly recommend `/ccgs:reverse-document` to verify
