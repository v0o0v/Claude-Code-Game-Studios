...
Use the template structure from `${CLAUDE_PLUGIN_ROOT}/templates/game-design-document.md`:
...
- `solo` → skip this agent spawn. Draft the section without the specialist. Add a note: "`ccgs:creative-director` not consulted — Solo mode. Review manually before production."
...
spawn `ccgs:creative-director` via Task:
...
**Do NOT draft Section B without first consulting `ccgs:creative-director`.** The framing
...
**Do NOT draft Section C without first consulting the appropriate specialists.** A `ccgs:systems-designer` reviewing rules and mechanics will catch design gaps the main session cannot.
...
**Review mode check** (apply before spawning):
- `solo` → skip this agent spawn. Draft the section without the specialist. Add a note: "`ccgs:systems-designer` not consulted — Solo mode. Review manually before production."
- `lean` → skip unless this is a section with HIGH implementation risk (Sections D and H only). For other sections, draft without the agent.
- `full` → spawn as described below.

**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, spawn specialist agents via Task in parallel:
- **Always spawn `ccgs:systems-designer`**: provide Core Rules from Section C, tuning goals from user, balance context from dependency GDDs. Ask them to propose formulas with variable tables and output ranges.
- **For economy/cost systems, also spawn `ccgs:economy-designer`**: provide placement costs, upgrade cost intent, and progression goals. Ask them to validate cost curves and ratios.
...
**Agent delegation (MANDATORY)**: Spawn `ccgs:systems-designer` via Task before finalising edge cases. Provide: the completed Sections C and D, and ask them to identify edge cases from the formula and rule space that the main session may have missed. For narrative systems, also spawn `ccgs:narrative-director`. Present their findings and ask the user which to include.
...
**Agent delegation**: If formulas are complex, delegate to `ccgs:systems-designer`
...
**Review mode check** (apply before spawning):
- `solo` → skip this agent spawn. Draft the section without the specialist. Add a note: "`ccgs:qa-lead` not consulted — Solo mode. Review manually before production."
- `lean` → skip unless this is a section with HIGH implementation risk (Sections D and H only). For other sections, draft without the agent.
- `full` → spawn as described below.

**Agent delegation (MANDATORY)**: Spawn `ccgs:qa-lead` via Task before finalising acceptance criteria. Provide: the completed GDD sections C, D, E, and ask them to validate that the criteria are independently testable and cover all core rules and formulas. Surface any gaps or untestable criteria to the user.
...
For **Visual/Audio** (non-required systems): Coordinate with `ccgs:art-director` and `ccgs:audio-director` if detail is needed. Often a brief note suffices at the GDD stage.
...
For **UI Requirements**: Coordinate with `ccgs:ux-designer` for complex UI systems.
...
Before finalizing the GDD, spawn `ccgs:creative-director` via Task using gate **CD-GDD-ALIGN** (`${CLAUDE_PLUGIN_ROOT}/docs/director-gates.md`).
...
|----------------|---------------|---------------------|
| **Foundation/Infrastructure** (event bus, save/load, scene mgmt, service locator) | `ccgs:systems-designer` | `ccgs:gameplay-programmer` (feasibility), `ccgs:engine-programmer` (engine integration) |
| Combat, damage, health | `ccgs:game-designer` | `ccgs:systems-designer` (formulas), `ccgs:ai-programmer` (enemy AI), `ccgs:art-director` (hit feedback visual direction, VFX intent) |
| Economy, loot, crafting | `ccgs:economy-designer` | `ccgs:systems-designer` (curves), `ccgs:game-designer` (loops) |
| Progression, XP, skills | `ccgs:game-designer` | `ccgs:systems-designer` (curves), `ccgs:economy-designer` (sinks) |
| Dialogue, quests, lore | `ccgs:game-designer` | `ccgs:narrative-director` (story), `ccgs:writer` (content), `ccgs:art-director` (character visual profiles, cinematic tone) |
| UI systems (HUD, menus) | `ccgs:game-designer` | `ccgs:ux-designer` (flows), `ccgs:ui-programmer` (feasibility), `ccgs:art-director` (visual style direction), `ccgs:technical-artist` (render/shader constraints) |
| Audio systems | `ccgs:game-designer` | `ccgs:audio-director` (direction), `ccgs:sound-designer` (specs) |
| AI, pathfinding, behavior | `ccgs:game-designer` | `ccgs:ai-programmer` (implementation), `ccgs:systems-designer` (scoring) |
| Level/world systems | `ccgs:game-designer` | `ccgs:level-designer` (spatial), `ccgs:world-builder` (lore) |
| Camera, input, controls | `ccgs:game-designer` | `ccgs:ux-designer` (feel), `ccgs:gameplay-programmer` (feasibility) |
| Animation, character movement | `ccgs:game-designer` | `ccgs:art-director` (animation style, pose language), `ccgs:technical-artist` (rig/blend constraints), `ccgs:gameplay-programmer` (feel) |
| Visual effects, particles, shaders | `ccgs:game-designer` | `ccgs:art-director` (VFX visual direction), `ccgs:technical-artist` (performance budget, shader complexity), `ccgs:systems-designer` (trigger/state integration) |
| Character systems (stats, archetypes) | `ccgs:game-designer` | `ccgs:art-director` (character visual archetype), `ccgs:narrative-director` (character arc alignment), `ccgs:systems-designer` (stat formulas) |
...
> open a fresh Claude Code session and run `/ccgs:design-system [system-name]` — it will
